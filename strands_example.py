...
config = LocalAgentConfig(
    system_instructions=(
        "You are an AWS engineer. Before calling any AWS SDK method, query the "
        "Stigmer MCP server for the method contract: required params, IAM "
        "permissions, pagination contract, and any known traps. For multi-step "
        "workflows, look for the generated least-privilege IAM policy. "
        "Write back fixes when you hit a trap not in the network."
    ),
    mcp_servers=[
        McpHttpServer(name="stigmer", url="https://stigmer.network/mcp"),
    ],
)
